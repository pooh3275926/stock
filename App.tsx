...
const BottomNav: React.FC<{ setPage: (page: Page) => void; currentPage: Page }> = ({ setPage, currentPage }) => (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-light-card dark:bg-dark-card border-t border-light-border dark:border-dark-border flex justify-around p-2 z-20">{navItems.map(item => (<button key={item.id} onClick={() => setPage(item.id as Page)} className={`flex flex-col items-center justify-center w-16 h-16 rounded-lg transition-colors ${ currentPage === item.id ? 'text-primary' : 'text-light-text/70 dark:text-dark-text/70'}`}><item.icon className="h-6 w-6 mb-1" /> <span className="text-xs">{item.label}</span></button>))} </nav>
);
